...
-- normalized phone/email within the agreed attribution window).
alter table leads
  add column closed_at timestamptz,
  add column closed_value numeric(12,2),
  add column closed_ref text;  -- invoice / PO number

-- Monthly web-attributed revenue rollup (basis for performance reporting).
...
select
  ...
  round(sum(closed_value) * 0.01, 2)   as commission_at_1pct
from leads
